...
async def db_get_sessions(db: AsyncIOMotorClient) -> List[SessionDatabase]:
    ...
    session_docs = db[session_collection_name].find()
...
async def db_create_session(session: SessionCreate, db):
    new_session = await db[session_collection_name].insert_one(session)
    return await db[session_collection_name].find_one({"_id": new_session.inserted_id})

async def db_get_session_by_id(id:str, db: AsyncIOMotorClient)->SessionDatabase:
    session : SessionDatabase = await db[session_collection_name].find_one({"_id": ObjectId(id)})
    return session
...
async def db_update_session_by_id(id:str, req: SessionUpdate, db:AsyncIOMotorClient):
    ...
    update = await db[session_collection_name].update_one({"_id": ObjectId(id)}, {"$set": request})
    if update:
        return True
    return False

async def db_delete_sessions_by_id(id: str, db:AsyncIOMotorClient) -> bool:
    delete = await db[session_collection_name].delete_one({"_id": ObjectId(id)})
