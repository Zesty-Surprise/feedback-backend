...
async def db_get_user_by_email(db:AsyncIOMotorClient, email:str)->User:
    user : User = await db[database_name][user_collection_name].find_one({"email": email})
    return user
...
async def db_get_all_users(db:AsyncIOMotorClient):
    users: list[ReadUser] = []
    user_docs = db[database_name][user_collection_name].find()
...
async def db_delete_user(email:str, db: AsyncIOMotorClient):
    delete = await db[database_name][user_collection_name].delete_one({"email": email})
